...
export const PINNED_DECISIONS: Record<string, string> = {
  ...
  "key-material-ownership":
    ...
    "In the §11.1 asynchronous companions both index keys are erased before the derivation is queued -- the Argon2id salt is derived synchronously, so nothing past that point reads them -- and the normalized value is copied rather than borrowed across the await, because under `identity` it is the caller's own array and on the marker path it is the process-wide reserved preimage; neither is ever zeroed.",
};
