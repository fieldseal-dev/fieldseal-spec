# fieldseal (Python core) reference

The detail behind [`README.md`](README.md): the conformance status of this core,
the behaviours it pins where the specification leaves a choice open, and what
its tests do and do not prove. The README is the place to start; this file is
for reviewers and contributors. The design it implements is
...
- **Blind indexes are bytes-in/bytes-out.** `nfc-casefold-v1` over bytes
  ...
  `CONFIGURATION_ERROR`, never a default. `nfc-casefold-v1` uses
  vendored Unicode 17.0.0 tables (`fieldseal.UNICODE_VERSION`) for both NFC and
  case folding, byte-identical to the TypeScript core's, so an index value does
  not depend on the interpreter's `unicodedata`. That closed D-10, which this
  file used to describe as open: CPython 3.14 ships Unicode 16.0, so folding
  with the interpreter's tables was a cross-core risk for shared indexes.
