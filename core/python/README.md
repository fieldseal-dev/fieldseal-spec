...
| | |
|---|---|
| Vector suite | 42/42 pinned results pass (30 vectors; `envelope/` counted in both directions, `blind-index/` also end to end — see `harness_notes` in the report); 1 family held out; both §3.5 out-of-band checks pass |
| Gate, parity and totality tests | 90 pass (`tests/test_gates.py`, `tests/test_parity.py`) |
