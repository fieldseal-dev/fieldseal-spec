...
| `GROUP BY` / `DISTINCT` / `ORDER BY` / aggregate or function expressions on the **ciphertext** column | **No** — a randomized suite writes a different envelope for every row, so every envelope is distinct: grouping returns one group per row (wrong counts under group keys that decrypt identically), `DISTINCT` deduplicates nothing, `ORDER BY` produces a stable-looking order with no meaning, and an aggregate computes on bytes — `MIN()` returns whichever envelope sorts first, *decrypted cleanly and presented as the minimum*. (Row added by G20, [#80]; each failure was measured against a shipping adapter before this row was written.) | Materialize → decrypt → sort, group, deduplicate or aggregate in application code; for range-like ordering, the coarse plaintext bucket column of the Range row. |
...
- **All ORMs:** ordering, grouping, `DISTINCT`, or aggregate/function expressions over a ciphertext column. `ORDER BY` sorts envelope bytes — a meaningless but stable-looking order; `GROUP BY` and `DISTINCT` treat every randomized envelope as distinct, returning wrong counts and deduplicating nothing; an aggregate computes on bytes, and the failure can be *silent and plausible* — `MIN()` over a ciphertext column returns whichever envelope sorts first, which decrypts cleanly and is presented as the minimum. An adapter MUST throw where its interception surface reaches the shape, and MUST document the shapes that surface cannot reach — raw SQL is already documented as such below, and declaration-level ordering an ORM applies inside its own compiler (Django's `Meta.ordering`) is reported by a startup check where a query-time refusal cannot see it. The Prisma `orderBy` sentence above is the named instance of this rule. *(Generalized from the Prisma bullet by G20, [#80](https://github.com/fieldseal-dev/fieldseal-spec/issues/80); previously stated for one ORM only.)*
