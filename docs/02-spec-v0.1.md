...
### 3.6 Plaintext rendering of logical types (normative)

§3.1–§3.5 pin every byte from the plaintext onward. This section pins the plaintext itself: the bytes an application value becomes before it reaches `encrypt()` or `blind_index()`. The core API takes bytes (§11.1) and never sees a logical type, so rendering is the adapter's obligation, in the same way §11.3 makes cryptography the core's.

**The vocabulary is closed.** An adapter MUST map every encrypted column to exactly one of the eight logical types below, at declaration time, and MUST refuse at declaration time a column whose application type it cannot map. It MUST NOT render any other type through a platform's default text conversion. Extending the vocabulary is a specification change with vectors, one type at a time.

| Logical type | Rendering (the plaintext bytes) | A writer MUST refuse |
|---|---|---|
| `string` | The UTF-8 encoding of the value's Unicode scalar values, byte for byte. No normalization — normalization belongs to the index (§7.2), not to the stored value. | A value that is not a sequence of Unicode scalar values (an unpaired UTF-16 surrogate). |
| `bytes` | The value itself. | — |
| `int` | ASCII decimal: `0`, or `-`? followed by a nonzero digit and further digits. No `+`, no leading zeros, no `-0`, no whitespace. Unbounded. | — |
| `decimal` | **Canonical by value:** ASCII `-`? `int-part` (`.` `frac`)?, where `int-part` has no leading zeros (`0` when the integer part is zero), `frac` has no trailing zeros and is omitted when empty, there is no exponent, and zero is `0`, never `-0`. Two decimals equal in value render identically: `1.50`, `1.5` and `15E-1` all render `1.5`, and `1E+2` renders `100`. | NaN and ±Infinity. |
| `float` | An IEEE 754 binary64 value, rendered as ECMAScript's `Number::toString(x)` in radix 10 (ECMA-262, the Number type's `toString` abstract operation), except that negative zero renders `-0`. | NaN and ±Infinity. |
| `boolean` | ASCII `true` or `false`. | — |
| `date` | A calendar date with no time zone, as RFC 3339 `full-date`: `YYYY-MM-DD`, year `0001`–`9999`. | A value outside that range. An adapter whose platform has no calendar-date type states the convention it uses instead (Prisma's: an instant at 00:00:00.000 UTC) and MUST refuse a value that is not exactly at that convention's midnight, rather than truncating it. |
| `datetime` | An instant, in UTC: `YYYY-MM-DDTHH:MM:SS.ffffffZ` — RFC 3339 `date-time` with exactly six fractional digits, uppercase `T` and `Z`, year `0001`–`9999`, no leap second. | A value with no time zone (a naive datetime) — never assume one — and a value outside the year range. |

**Readers are exactly as strict as writers.** A reader MUST refuse plaintext that is not the canonical rendering for the column's declared type, rather than accepting a looser form of it. For `float`, a reader parses the bytes, re-renders the parsed value, and refuses unless the result matches byte for byte. A reader whose platform type cannot hold a canonical value exactly MUST refuse it rather than round, truncate, or widen it: an `int` beyond the platform integer, or a `datetime` whose sub-millisecond digits are nonzero on a platform with millisecond instants. Refusal on either side raises the adapter's not-supported error, not a §9 core error: nothing about the envelope is wrong.

**Changing a column's declared type is a change of plaintext encoding.** Every row already written carries the old rendering, and a blind index over it (§7.8) indexes the old bytes. It needs a backfill, not an edit.

*Justification.* Before this section, both shipped adapters rendered values their own way and disagreed on four of the eight types (G25, [#123](https://github.com/fieldseal-dev/fieldseal-spec/issues/123)). Each disagreement is a different failure class. A `date` written by Django was read by Prisma as an instant and rewritten as one, after which Django could no longer read the row, and anyone west of UTC saw the previous day. A `decimal` had no Prisma declaration at all, so it went through a binary64 and changed value. A `boolean` was refused loudly in both directions.

The measurements also turned up failures within a single adapter, which is why the rules are canonical rather than merely agreed:

- Django rendered `Decimal("1.5")` and `Decimal("1.50")` as different bytes. A blind index is derived from those bytes (§7.2), so an equality lookup for one silently missed rows written as the other. That is a false negative, which §7.5's re-verification cannot repair, because the missed row is never a candidate.
- A binary64 renders differently in CPython (`1e+16`, `1e-07`) and in ECMAScript (`10000000000000000`, `1e-7`).
- Node's `Date` truncates `.123456` to `.123` without error.
- Node's UTF-8 encoder replaces an unpaired surrogate with U+FFFD without error.

Every one of these decrypts cleanly into a wrong value, the silent class §10.2 exists to prevent. The value-canonical `decimal` gives up scale (`1.50` is read back as `1.5`) so that equality over the index agrees with SQL numeric equality. ECMAScript's `Number::toString` is chosen for `float` because it is fully specified, shortest-round-trip, and already what one of the two target platforms does. Negative zero is excepted because ECMAScript renders it as `0`, which would silently discard the sign. Six fractional digits is the finest precision any target platform holds, and requiring the digits rather than trimming them makes the rendering one string per instant. The `date` and `datetime` year range is the one every target platform can represent.

**What this does not pin.** Types outside the vocabulary — UUIDs, times of day, durations, JSON documents, network addresses — have no rendering here, so an adapter refuses them rather than inventing one. An application that needs one encrypted stores it as a `string` it renders itself, and owns that rendering.

The `codec/` vector family (§12) pins every rule in this section, in both directions and with the refusals. It binds adapters, not cores: `MANIFEST.json` lists it under `adapter_files`, which a core's conformance run does not iterate (`docs/08` §4.8).
...
A blind index MUST be treated as a **filter**, never as an answer. After retrieving candidate rows by index, the implementation MUST decrypt the candidates and compare **under the index's own equality**: `normalize(stored_plaintext)` against `normalize(queried_value)`, using the normalizer declared for the index that produced the candidates (§7.2). The comparison is on the normalizer's output bytes. If the declared normalizer *refuses* a value rather than transforming it, that side of the comparison MUST fall back to the value's raw plaintext bytes; two refused values are therefore equal only byte-for-byte. *(The equality was unstated until G19 ([#78](https://github.com/fieldseal-dev/fieldseal-spec/issues/78)), resolved 2026-08-26; the previous wording — "compare the actual values" — admitted a byte-exact reading under which the two answers differ on any normalized column.)*

*Justification.* §7.2 defines the index over `normalize(plaintext)`, so its equivalence classes **are** normalization classes; re-verification exists to remove the collisions §7.4's truncation mandates, not to reintroduce a distinction the declared normalizer deliberately erased. Under a byte-exact reading, a folding normalizer enlarges every candidate set, coarsens the §7.4 bucket, and buys no query capability — strictly worse than declaring no transformation at all, on exactly the email-like columns §7.2's example points folding at. And canonical equivalence decides the remaining case: Unicode normalization (UAX #15) exists so that byte-different, canonically equivalent spellings — `U+00E9` versus `e` + `U+0301` — are one text to every reader; a comparison that distinguished them would contradict the normalizer it had just applied. The `blind-index/` vector family pins the index half of this rule with explicit collision pairs, including that canonical-equivalence pair.

A consequence adapters MUST document: on a column whose declared normalizer is anything but the identity transformation, an equality lookup is equality **under that normalizer**, not byte equality of the plaintext — a query for `ada@example.com` can return a row whose stored value renders as `Ada@Example.com`. An adapter MUST NOT offer a second, differently-folded equality over such a column (a case-insensitive lookup beside `exact`, for example): the column has exactly one equality, and its index can serve only that one. Where an implementation additionally indexes values the declared normalizer refuses, under a shared reserved marker (an architecture-level mechanism outside this specification), two different refused values share one index value — and the raw-bytes fallback above is what separates them, so there the comparison is load-bearing rather than merely collision-trimming.
...
| Negated membership (`NOT IN`) and negated equality (`!=`, `NOT (col = v)`) | **No** — and this is not the same "no" as the rows below, which are unsupported because the index cannot answer them at all. The positive forms above are supported *because* §7.5 re-verifies: §7.4 mandates collisions, so an index predicate returns a **superset** and the surplus rows are present to be inspected and dropped. Negation inverts that. `NOT IN (bidx(v₁)…bidx(vₙ))` excludes every row in those buckets — including rows whose plaintext is none of `v₁…vₙ` and which belong in the answer. Those rows never reach the client, so §7.5 has nothing to put back, and the query silently drops correct results in proportion to `P × 2^(−b)`: the collision rate §7.4 deliberately requires. **A filter's false positives are recoverable; an exclusion's false negatives are not.** (Row added by G21, [#87](https://github.com/fieldseal-dev/fieldseal-spec/issues/87). **No documented bucket-semantics escape hatch lifts this** — §10.2's negation clause carries the rule and the reasoning, decided by G24, [#100](https://github.com/fieldseal-dev/fieldseal-spec/issues/100).) | Fetch the matching rows with the positive form, re-verify them under §7.5, and exclude their primary keys in application code; or materialize the candidate set and filter after decryption. |
| Prefix | Gated, §7.9 | — |
| `GROUP BY` / `DISTINCT` on the blind-index **sibling** column | Yes, with the caveat that it groups by index value, including the §7.4 collisions | — |
| Non-null count (`COUNT(col)`, non-distinct, single column) on the **ciphertext** column | **Yes** — conditional on §10.2's NULL-preservation invariant: the SQL reads null-ness, never envelope bytes, so the count over envelopes equals the count over plaintexts exactly. `COUNT(DISTINCT col)` is **No** — every envelope is distinct, so it counts rows. (Row added by G23, [#89].) | — |
| `GROUP BY` / `DISTINCT` / `ORDER BY` / aggregate or function expressions that read envelope bytes on the **ciphertext** column | **No** — a randomized suite writes a different envelope for every row, so every envelope is distinct: grouping returns one group per row (wrong counts under group keys that decrypt identically), `DISTINCT` deduplicates nothing, `ORDER BY` produces a stable-looking order with no meaning, and an aggregate that reads bytes computes on them — `MIN()` returns whichever envelope sorts first, *decrypted cleanly and presented as the minimum*. The non-null count is the row above, not this one: it reads no envelope bytes. (Row added by G20, [#80]; each failure was measured against a shipping adapter before this row was written. Discriminator narrowed to bytes-reading by G23, [#89].) | Materialize → decrypt → sort, group, deduplicate or aggregate in application code; for range-like ordering, the coarse plaintext bucket column of the Range row. |
...
| Range, `<`, `>` | **No** | A deliberately coarse plaintext bucket column (age decade, income band) with its own documented risk assessment, plus exact filtering after decryption. This is honest; OPE is not. |
...
- **All ORMs — negation over a blind index (normative):** an adapter MUST reject a negated membership or negated equality over an encrypted column (`notIn:`, Prisma's scalar `.not`, Django's `exclude()`, SQL `NOT IN` / `!=`), whether or not the column declares an index. It MUST NOT rewrite the predicate onto the index. §7.10's row states why: the exclusion is computed in the database over §7.4 buckets, so the rows it wrongly removes never reach the client and §7.5 has nothing to put back — the answer is silently short, by the collision rate the truncation band mandates. This is the same asymmetry the positive rewrite depends on, run backwards. **A documented bucket-semantics escape hatch (Django's `.candidates()`, Prisma's `candidateScope()`) MUST NOT lift this refusal.** A hatch transfers §7.5 to the caller, and §7.5 is a *filter* obligation: it is dischargeable from a superset — decrypt the candidates, drop the surplus — and is not dischargeable from an exclusion, because the rows the database removed are not among the ones the caller was handed. The prohibition is scoped by **position, not by operator**: an encrypted-column predicate MUST be refused wherever a *wider* index match set yields a *narrower* result. That covers negated membership and negated equality, a negated group (`NOT (…)`, Django's `~Q`), a relation filter that is itself a negation (Prisma's `none` and `isNot`), and `XOR`, which is a negation once expanded. It does not cover positions where a wider match set yields a wider result — `AND`, `OR`, `some`/`every`/`is` — which a hatch MAY lift, because there the caller is handed a superset and §7.5 stays dischargeable. It does not reach `IS NULL` / `IS NOT NULL` either: under the NULL-preservation invariant below those read the column's own null-ness, touch no index bucket, and are exact under negation. **An adapter's refusal message for a covered shape MUST NOT direct the caller to its escape hatch** — a caller who follows the error text would land on semantics that differ in kind from the filter case with nothing saying so, which is the state Django's `exclude()` message shipped in. *(Decided by G24, [#100](https://github.com/fieldseal-dev/fieldseal-spec/issues/100), 2026-09-06, in the direction the Prisma adapter had shipped; the Django adapter's `.candidates()` lifted the refusal and was changed to match.)* *(Stated by G21, [#87](https://github.com/fieldseal-dev/fieldseal-spec/issues/87), after `docs/13` §4 was found granting `notIn:` a rewrite that neither §7.10's table nor §10.2's `in:` permission ever granted. Both shipped adapters already refused it; the specification is what was wrong.)*
- **All ORMs — the NULL-preservation invariant (normative):** an adapter MUST store SQL `NULL` as `NULL` — never as an envelope — and a present value as a non-`NULL` envelope, with any blind-index sibling `NULL` exactly when its source column is. The empty string is a present value, not an absence: it encrypts to an ordinary envelope and the column is non-`NULL`. Absence is not a value and MUST NOT be encoded as ciphertext. This is what makes `IS NULL` / `IS NOT NULL` predicates and the non-null-count carve-out below exact over encrypted columns: the column's null-ness *is* the value's absence, with no collision class and nothing for §7.5 to re-verify. *(Stated by G23, [#89](https://github.com/fieldseal-dev/fieldseal-spec/issues/89): both shipped adapters already enforced and tested it, and the query shapes they served over null-ness silently rested on it being true without the specification saying so.)*
- **All ORMs:** ordering, grouping, `DISTINCT`, or aggregate/function expressions **that read envelope bytes** over a ciphertext column. `ORDER BY` sorts envelope bytes — a meaningless but stable-looking order; `GROUP BY` and `DISTINCT` treat every randomized envelope as distinct, returning wrong counts and deduplicating nothing; an aggregate that reads bytes fails *silent and plausible* — `MIN()` over a ciphertext column returns whichever envelope sorts first, which decrypts cleanly and is presented as the minimum. An adapter MUST throw where its interception surface reaches the shape, and MUST document the shapes that surface cannot reach — raw SQL is already documented as such below, and declaration-level ordering an ORM applies inside its own compiler (Django's `Meta.ordering`) is reported by a startup check where a query-time refusal cannot see it. The Prisma `orderBy` sentence above is the named instance of this rule. **One shape is carved out because it reads no envelope bytes:** the plain, non-distinct, single-column non-null count (`COUNT(col)`; Django's `Count(field)`, Prisma's `_count: { field: true }`) MAY be served, *conditional on the NULL-preservation invariant above* — the SQL reads null-ness alone, so the count over envelopes equals the count over plaintexts exactly. `COUNT(DISTINCT col)` is **not** carved out: it compares envelopes, so it counts rows — a silent-plausible wrong answer squarely inside this rule. An adapter that chooses to refuse the carved-out shape anyway remains conformant, but MUST NOT justify the refusal as bytes-computation — that stated reason is false for this shape, and a refusal carrying a false justification is how the two shipped adapters came to read this bullet opposite ways. *(Generalized from the Prisma bullet by G20, [#80](https://github.com/fieldseal-dev/fieldseal-spec/issues/80); previously stated for one ORM only. The bytes-reading discriminator, the `COUNT` carve-out and the refusal-honesty rule by G23, [#89](https://github.com/fieldseal-dev/fieldseal-spec/issues/89).)*
- **All ORMs — the codec is a site, not only the query (normative):** everything above is written about *query* shapes, but a wrong answer can also arrive through the codec that turns an application value into plaintext bytes, and never touch a query. An adapter MUST render and parse values exactly as §3.6 pins them. It MUST refuse a column whose type §3.6 does not cover, at declaration time. It MUST refuse — never coerce, round, truncate, or widen — a value on either path that §3.6's rules do not admit. *(Stated by G25, [#123](https://github.com/fieldseal-dev/fieldseal-spec/issues/123), after a `decimal` was measured changing value between the two shipped adapters with every envelope authentic and nothing raised.)*
...
- **Logical-type rendering (adapters):** for each §3.6 type, application value → expected plaintext bytes, and plaintext bytes → value or refusal — including every write-side refusal §3.6 names, non-canonical bytes a reader MUST refuse, and the platform-precision cases (a sub-millisecond `datetime`, an unpaired surrogate). These vectors bind adapters rather than cores and are listed under `MANIFEST.adapter_files`. An adapter MUST pass them to claim L1.
