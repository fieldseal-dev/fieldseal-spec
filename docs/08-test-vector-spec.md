...
- **`hmac-sha512` only, for now.** `idf_params` is present and empty so the slot exists; an `argon2id` index case lands when `blind-index/argon2id.json` leaves `MANIFEST.held_out`. Asserting Argon2id agreement on every merge while the manifest says the suite deliberately does not count that family would have the two saying different things about one primitive.

**Case set for the index half:** at least one case per normalizer the producer supports; at least one non-ASCII value; at least one pair that MUST collide (a case-fold or NFC pair); and the marker case wherever a column declares `on_unindexable: "bucket"`.
