...
- Four more arrive with suite `0.8.0-provisional` (G26, [#176](https://github.com/fieldseal-dev/fieldseal-spec/issues/176)), two per IDF: `invalid-utf8-high-surrogate-b15` and `invalid-utf8-low-surrogate-b15`, shape `"refuse"`. Each preimage is `a` + the generalized-UTF-8 encoding of a lone surrogate (`ED A0 80` for U+D800, `ED B0 80` for U+DC00) + `b`, passed to the public `blind_index` operation **as bytes** under `on_unindexable = refuse`, and each MUST raise `INVALID_ARGUMENT` — docs/09 §7.1 clause 5's strict decode. They fill the obligation G15 recorded and never shipped ("an invalid UTF-8 byte sequence and a lone surrogate, asserting `INVALID_ARGUMENT`"), and they are the premise docs/14 §4's representability route rests on: in a language whose string type is bytes (Go), this is the only form a lone surrogate can take. A vector pins the refusal's **code**, which is the same for both; whether the two refusals are distinguishable is a harness assertion (§5 item 9), not a vector.
