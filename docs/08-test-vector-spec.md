...
- Suite `0.8.0-provisional` (G26, [#176](https://github.com/fieldseal-dev/fieldseal-spec/issues/176)) adds a fourth one-sided assertion, `"refuse"`, in `blind-index/`: `inputs.preimage` is hex bytes (not valid UTF-8, so no JSON string could hold it), `inputs.on_unindexable` is `"refuse"`, and `expected.refuse` is the code the index path must raise. It carries no `index`. §4.4 has the vectors.
...
`kdf/index-key.json` mirrors it with `tenant_index_key` as input, `context.purpose` of the form `"index:<index-id>"`, `row_id` null in every vector except `row-id-dropped`, and `expected.index_key`; a top-level `index_id` repeats the identifier for readability only. The two halves of a vector describe different contexts: `context` is the **caller's** context, as passed to the core, while `expected.info` is the §7.2 **derivation** context, after `row_id` is dropped. A harness derives `info` through the index path, never by encoding `context` as given (§5 item 11); two harnesses did the latter and would have failed a correct core on `row-id-dropped` (#191). `index_key` is the check that tests the core: a harness computes `info` with its own drop, so the `info` comparison only tests that the vector file is consistent with itself. (Until suite 0.2.0 the context carried `purpose = "encrypt"` and a harness had to construct the index purpose itself — docs/18 D-06.) The file carries two vectors differing **only** in the index identifier (`email-eq` vs `ssn-eq`) to pin the §7.2 distinctness rule, two differing only in `column_uuid` to pin per-column separation, one whose caller context carries a `row_id` (marked `same_as` the row-less vector, since §7.2 drops it), and one at the maximum index-path context (G14).
...
- Four more arrive with suite `0.8.0-provisional` (G26, [#176](https://github.com/fieldseal-dev/fieldseal-spec/issues/176)), two per IDF: `invalid-utf8-high-surrogate-b15` and `invalid-utf8-low-surrogate-b15`, shape `"refuse"`. Each preimage is `a` + the generalized-UTF-8 encoding of a lone surrogate (`ED A0 80` for U+D800, `ED B0 80` for U+DC00) + `b`, passed to the public `blind_index` operation **as bytes** under `on_unindexable = refuse`, and each MUST raise `INVALID_ARGUMENT` — docs/09 §7.1 clause 5's strict decode. They fill the obligation G15 recorded and never shipped ("an invalid UTF-8 byte sequence and a lone surrogate, asserting `INVALID_ARGUMENT`"), and they are the premise docs/14 §4's representability route rests on: in a language whose string type is bytes (Go), this is the only form a lone surrogate can take. A vector pins the refusal's **code**, which is the same for both; whether the two refusals are distinguishable is a harness assertion (§5 item 9), not a vector. Two fields are there for the declaration, not the outcome: `truncate_bits` (with the `-b15` id suffix that follows it, as elsewhere in the family) is carried because an index declaration is invalid without one, and nothing is truncated on a refusal. And a `refuse` vector has no positive control of its own: a core that refused *every* bytes input would pass it. The bytes-in `#pipeline` results of the primitive vectors are what rule that core out.
...
8. Assert the spec §3.5 plaintext length bound out-of-band, since it has no vector (G10): a test MUST show that 2³¹ bytes is refused with `LENGTH_EXCEEDED`, and that an envelope whose implied plaintext length (received length minus the suite's fixed overhead) is at least 2³¹ is refused with it too, and the harness MUST record the assertion in the report's `out_of_band` block (docs/14 §4) with its `basis`. **Directly** (`basis: "direct"`), the harness allocates or maps the operand and calls the public operation. **Through the seam** (`basis: "seam"`, G26), which any core MAY use and which a core whose byte type cannot hold the operand has no alternative to, it drives synthetic operands through the docs/09 §4 length seam, under the conditions docs/14 §4 sets. A harness that tried the direct route and could not allocate the input records `not-run` with the reason, rather than passing silently — the point of the block is that an unverified bound is visible in the report instead of absent from it.
9. Assert the docs/09 §7.1 lone-surrogate refusal out-of-band, since it cannot have a vector in every target language (G16 part A): a test MUST show that two *distinct* unpaired surrogates are both refused **and refused distinguishably**, and record it in `out_of_band` as `docs/09/7.1/lone-surrogate-refusal`, `basis: "direct"`. A harness in a language whose string type cannot represent an unpaired surrogate (Go, Rust) records it instead with `basis: "representability"` (G26, [#176](https://github.com/fieldseal-dev/fieldseal-spec/issues/176)), and `pass` only when all of these hold: the `blind-index/` `refuse` vectors of §4.4 are pinned and pass in this core; the harness shows that the two byte preimages those vectors carry (`ED A0 80` and `ED B0 80`, each between `a` and `b`) are refused with **distinguishable** diagnoses on the same path; and `method` states the premise — the language's string type cannot hold a lone surrogate, this core refuses invalid UTF-8 on its string path (docs/09 §7.1 clause 5), and the vector ids that pin it. Such a core's strict-UTF-8 diagnosis names the byte offset and the malformed sequence (at most four bytes) and nothing around it: error text is routinely logged, and a diagnosis that echoed its input would put plaintext there. Before those conditions hold, the entry is `not-run`. Same terms as item 8: an unverified requirement is visible in the report rather than absent from it.
...
11. For `kdf/index-key.json`: pass the vector's `context` to the core as the caller's context, and compare `expected.info` with the §7.2 derivation context (`purpose` retargeted, `row_id` dropped), never with `context` encoded as given (§4.2). Where the harness runs blind indexes through the public client (`#pipeline`), the caller's context carries a non-null `row_id`, so that the client's own drop is on the path the result depends on.
...
- Where an external, already-published vector exists for a primitive, the generator's primitive layer MUST be checked against it first. **Status of each source, re-verified 2026-08-22 (this bullet previously carried a flag saying they had not been):**
  ...
  - **AES-256-GCM** — NIST CAVP GCM vectors apply directly. **Wired 2026-09-10:** `tools/vector-gen/fieldseal_vectorgen/kat_aesgcm.py` vendors 50 cases from NIST's `gcmtestvectors.zip` (CAVS 14.0) in suite `0xFF01`'s shape — 256-bit key, 96-bit IV (spec §4.4), 128-bit tag (§4.5): Count 0 of each of the 25 plaintext-length × AAD-length sections of `gcmEncryptExtIV256.rsp`, and the first FAIL case of each section of `gcmDecrypt256.rsp`. On every run the generator checks both of its AES-GCM paths against them, and the run fails on any disagreement: `cryptography`'s `AESGCM`, which seals every envelope vector, and its own GHASH/tag/keystream (`gcm.py`, used only to build the salamander vector), which had previously been checked only for agreement with `cryptography` — a consistency check, not a CAVP one. The regenerated suite was byte-identical before and after.
...
**Status 2026-09-22 — suite `0.8.0-provisional` (G26, [#176](https://github.com/fieldseal-dev/fieldseal-spec/issues/176)): 150 vectors and 182 results, both cores green with identical synchronous ids.** The four new vectors are §4.4's `refuse` pair in each IDF family. The TypeScript core's second pass carries 182 `#async` results, of which 69 route through a companion (the four new ones among them) and 113 re-run the synchronous operation. `codec/logical-types.json` is unchanged apart from the version string.

**Status 2026-09-25 — suite `0.9.0-provisional` ([#191](https://github.com/fieldseal-dev/fieldseal-spec/issues/191)): 150 vectors and 182 results, unchanged in number.** One vector changes: `kdf/index-key/row-id-dropped` now carries the caller's `row_id` in `context` (`726f772d3432`, `row-42`). From 0.2.0 to 0.8.0 it carried `row_id: null`, because the generator wrote the context after §7.2's drop rather than before it, so its input equalled `email-eq`'s and its `same_as` held by construction: a core that kept `row_id` in the index-key `info` passed it. Its `expected` values are unchanged — they are the row-less ones, which is the point. `context` in `kdf/index-key.json` is therefore the caller's context and `expected.info` the §7.2 derivation context; a harness derives `info` through the index path (§4.2, §5 item 11). The generator now asserts that a `same_as` vector's inputs differ from its target's.
