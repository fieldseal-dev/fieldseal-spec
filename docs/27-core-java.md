...
Fieldseal fs = Fieldseal.builder()
    ...
    .cachePolicy(new CachePolicy(             // required with the envelope provider, refused otherwise
        ...
        10_000))                              // capacity
    .warmExecutor(warmPool)                   // envelope provider only; default: daemon threads, not the common pool
...
- **Where `warm` blocks.** The envelope provider's `warm` calls the key store and the KMS, which block, so it runs them on the builder's `warmExecutor` ([#192](https://github.com/fieldseal-dev/fieldseal-spec/issues/192)). By default that is a pool of daemon threads named `fieldseal-warm-N`, shared by every client, one thread per `warm` in progress, each gone after a minute idle. It is never the ForkJoin common pool: that pool's threads belong to the application's other async work, and on two CPUs or fewer `CompletableFuture` uses a new thread per task instead of it. A `warm` still loads its slots one after another; loading them in parallel is a separate change. An executor that rejects the task fails `warm`'s future.
