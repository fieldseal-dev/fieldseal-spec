...
**Naming note:** `Fieldseal` was selected 2026-08-08 after collision vetting; the earlier working name `OpenFLE` was dropped for one-letter confusability with OpenFHE, the prominent homomorphic-encryption library. As of 2026-08-08 the npm (`fieldseal` and the `@fieldseal/*` scope) and PyPI (`fieldseal`) names are claimed with placeholders, the GitHub org is `fieldseal-dev`, and crates.io / NuGet / Maven Central remain free. Still outstanding before public release: the PyPI `field-seal` upload, domain registration (`fieldseal.dev`, also required for the `dev.fieldseal` Maven groupId), and a trademark search of Classes 9 and 42 in USPTO TMSearch — which replaced TESS, named in earlier drafts. The [README](../README.md) links the four published packages; the fuller namespace status table it carried until 2026-09-25 (domains, other registries, the `field-seal` upload refusal) is in that file's history, and the checklist behind it is `internal/naming-tasks.md`.
...
- **Gate 0b — permission to freeze.** Unchanged from the original criterion: at least two people with real cryptographic credentials have read the spec and their objections are addressed or documented as open questions. **Until this closes, the project MUST NOT** assign a non-provisional suite identifier, publish the vector suite as anything but `-provisional`, describe any implementation as conformant to a frozen format, invite production adoption, publish any package at version 1.0 or above, or run the Phase 3 dissemination track (§5 of `docs/05-dissemination.md` and beyond). Gate 0b is not deferred, weakened, or satisfiable by self-review.
- **Experimental releases — permitted before Gate 0b (added 2026-09-18).** The project MAY publish its cores and adapters to package registries, for evaluation and non-production use, while Gate 0b is open. Every such release MUST meet all of the following, and one that misses any of them is not an experimental release but a breach of Gate 0b:
  1. **Below 1.0**, with the registry's own maturity marker set where it has one (PyPI: a `Development Status` classifier of `3 - Alpha` or lower).
  2. **Provisional suites only.** Every registered suite is in spec §4.8's reserved range, and the §4.8 arming gate ships exactly as specified — never armed by default, never armed by the package's own configuration or examples.
  3. **The warning comes first.** The package README and the registry's short description open by stating that the constructions have not been independently reviewed, that the format may change before 1.0, that data written under a provisional suite may have to be re-encrypted if Gate 0b changes a construction, and that the package is not for production data.
  4. **The honest limitations travel with it.** The README states every limitation the specification says an implementation or adapter MUST document. The core set is spec §2 (the threat model, including §2.3 on logs), §3.3, §5.5, §7.3 and §8.1, and an adapter adds the ones §7 and §10.2 place on it.
  5. **No overclaiming anywhere.** No release note, package metadata or announcement describes the package as secure, reviewed, production-ready, or conformant to a frozen format.

  What is invited is evaluation, feedback and non-production use. Production adoption is not, and stays behind Gate 0b.
...
*Why experimental releases are not a loophole either (2026-09-18).* Four weeks after the split, reviewer recruitment has still not succeeded. The evidence is the outreach log in `docs/16-reviewer-brief.md` §4. The choice was between waiting indefinitely with finished code unpublished, and publishing it under the controls this project already built for exactly this state. Nothing about the freeze moves: no suite identifier is assigned, the vector suite stays `-provisional`, nothing reaches 1.0, and production adoption is not invited. What changes is that code reaches people who can evaluate it. Spec §4.8, not a README paragraph, is what keeps an experimental release out of production data: writing refuses unless the operator arms provisional use, and anything written after arming can be identified from its stored bytes without a key. The residual risk is that someone arms anyway and puts real data behind a construction that later changes. The conditions above make sure that was their informed choice rather than a surprise, and §4.8's recovery property bounds what it costs them. One side effect, stated without overclaiming: the one funded-audit broker `docs/16` §4 examined (item 5, OSTIF) mostly publishes reviews of shipping code rather than specifications, so a published package may be eligible for a route that a specification is not. That makes a route eligible; it does not fund one.
...
|---|---|---|
| **The spec is wrong in a way only a cryptographer would catch.** | High likelihood, catastrophic impact. 37signals shipped a deterministic-encryption flaw and an RCE with better resources than this project has. | Independent security review is a **release gate**, not a nice-to-have — it is Gate 0b (§8), and no suite freezes, no vector suite is published as stable, nothing reaches 1.0, and no production adoption is invited without it. Freeze suites rather than offering agility. Copy proven constructions (Tink, AWS ESDK, Vault) rather than inventing. **Gate 0a's residual risk, stated plainly:** implementation proceeds against constructions that are provisional, so a reviewer finding forces regenerated vectors and a retired suite identifier. Spec §4.8 bounds that cost — provisional identifiers live in a reserved range, so data written under one is identifiable from stored bytes alone, and cores refuse to write under one unless the operator explicitly armed it. **Experimental releases' residual risk (2026-09-18):** published pre-1.0 packages put that refusal in front of people outside the project, and an operator who arms it anyway has real data behind an unreviewed construction. The release conditions in §8 make that an informed choice, and §4.8's reserved range makes the affected rows findable. |
