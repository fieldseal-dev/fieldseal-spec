...
**[KNOWN GAP — G20 ([#80](https://github.com/fieldseal-dev/fieldseal-spec/issues/80))] Ordering, grouping and `DISTINCT` over the ciphertext column are currently served silently.** `order_by("email")` returns envelope-byte order (meaningless, stable-looking); `values("email").annotate(Count(...))` groups by envelope bytes, so every row is its own group and the counts are wrong numbers under group keys that print identically; `earliest("email")` returns an arbitrary row. Spec §7.10 rules `ORDER BY` out and §10.2 names the throw — for Prisma only, which is the gap. The refusal design (every `FieldsealQuerySet`, obligations or none; `.candidates()` does not lift it; checks for `Meta.ordering` and admin sortable columns) is in the issue and lands with its closure.
