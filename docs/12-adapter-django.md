...
| subquery embedding (`__in=qs`, `Subquery`, `Exists`) | **refused, at two layers** | the subquery runs entirely in the database, where §7.5 cannot run, so the outer query would receive unverified candidates — silently. `resolve_expression` catches the route where Django resolves the *queryset*; `Exists(qs)` and `Subquery(qs)` take `qs.query` in their constructor and never call it, so `annotate()`/`alias()` carry the second layer (found in the G24 review round — that route had been reaching SQL unrefused). Escape: materialize verified pks, or embed `.candidates()` **in a positive `filter()`** — a subtractive position is refused there too, see below |
