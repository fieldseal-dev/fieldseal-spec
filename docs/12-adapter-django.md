...
**The residue, stated rather than claimed away.** A `.candidates()` bucket in a *subtractive operand* — `exclude(pk__in=Owner_qs.candidates())` — is refused on any `FieldsealQuerySet` by the bucket-as-operand rule above, and has no compile-time backstop to fall to: the outer predicate is on a plain column, so no `_IndexedLookup` compiles anywhere in that statement and the lookup layer is never consulted. From `Model._base_manager` there is therefore nobody to read the position, and the shape is served, short by the collision. That is the same residue this section already records for a model with no encrypted column of its own, with the owning model's plain manager as a second instance; it stays a residue rather than a hole for the same reason, which is that the caller had to construct the bucket deliberately to arrive there. It is asserted as a residue in the test suite so that it cannot drift back into a claim.
