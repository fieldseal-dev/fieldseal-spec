...
| Argon2id raw output | `argon2-cffi` | `argon2.low_level.hash_secret_raw(secret=…, salt=…, time_cost=…, memory_cost=…, parallelism=…, hash_len=32, type=Type.ID)` provides raw output with explicit salt — the IDF shape §7.2 needs. **Two traps, verified 2026-08-22.** (a) argon2-cffi's `secret=` is the **password**, *not* Argon2's optional secret key `K`; the names collide and the collision is exactly the kind that produces a silent cross-implementation divergence. (b) **Argon2's `K` is not reachable from any supported argon2-cffi API** — only via the ultra-low-level `core()` call with hand-built CFFI structs, which argon2-cffi's own documentation warns against. G2's proposed invocation layout routes `index_key` through `K`, so **this row cannot implement G2 as proposed.** See the portability finding in [`G02`](../docs/issues/G02-argon2id-parameters.md) |
