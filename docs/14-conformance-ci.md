...
*Delta, updated 2026-08-31:* what ships is still **one** workflow, `conformance.yml`, now covering `typescript-core`, `python-core`, `cross-core-result-ids`, `django-adapter` (SQLite and Postgres), `prisma-adapter` (SQLite and Postgres), `vectors-reproducible`, `suite-integrity`, `unicode-tables`, `cross-produce`/`cross-consume` and `python-lint`. `cross-core-result-ids` was added 2026-09-04 and is the one job that reads *two* reports: it compares the cores' synchronous result-id sets, which `docs/08` §414 had claimed since the second core shipped with nothing enforcing it. The per-core jobs cannot catch what it catches — each compares a core against the vectors that core chose to run, so a core that stopped running a family entirely stays green. It also holds `async_companions` to §4's definition per report, which is what keeps its own `#async` filter honest. The per-file split this section anticipated "when adapters arrive" did **not** happen when they arrived, and on the evidence it should not: one workflow keeps the cross jobs' `needs:` edges in one file, which is where the ordering that matters lives. There is no `cross.yml`; the cross jobs run inside `conformance.yml` on every push and on the nightly schedule (catching toolchain drift — a Node or OpenSSL upgrade changing behaviour is exactly the class of bug this project exists to surface).
