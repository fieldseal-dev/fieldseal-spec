...
- **Plaintext length bounds:** normative as of spec §3.5 (G10, issue #10, resolved 2026-08-09) — reject plaintexts > 2³¹−1 bytes at the API boundary with `LENGTH_EXCEEDED`, and reject on decrypt before allocating when an envelope implies more than the bound. The decrypt-side check reads the received byte count and the suite's fixed overhead only, so it needs no key or context and does not interact with the G5 ordering question. The bound is a ceiling, not a support guarantee: a runtime that fails below it with an allocation error is still conformant, but a 2³¹-byte input MUST produce `LENGTH_EXCEEDED` rather than the platform's error. **Per-binding obligation:** each core's binding doc states its platform's largest byte buffer, measured on the runtime the core targets, and says whether that ceiling or this bound binds first. This is a per-core fact, so it is owed by the binding docs rather than listed here — `docs/10` §5 and `docs/11` §5 carry the Phase 1 halves; the Java, .NET and Go halves are owed by those cores' documents when Phase 2 opens, and spec §3.5 already states the JVM case qualitatively (`byte[].length` is an `int`, so the platform binds by construction). **The length seam (G26, [#176](https://github.com/fieldseal-dev/fieldseal-spec/issues/176)).** A core MAY route the bound through one internal operand type whose length is a 64-bit integer: each public `encrypt`, `decrypt` and `rotate` entry point constructs it exactly once from the caller's bytes, every later step reads the operand through it, and the guard reads only its length. A test can then drive synthetic lengths through the same path every public call takes, which is what docs/14 §4's `basis: "seam"` requires. A core whose byte type cannot hold a 2³¹-byte operand (`int` array lengths on the JVM and .NET) has no other way to show the guard fires, so for it the seam is part of the core, not an option (`docs/26` §4); a core that can allocate the operand MAY still prefer it to a 2-GiB test. *(Reworded 2026-09-20, issue [#167](https://github.com/fieldseal-dev/fieldseal-spec/issues/167): the **[VERIFY]** this replaces asked for "each Phase 1 language" and then listed Node, the JVM and .NET — omitting Python, which is Phase 1, and naming two languages this document's own header calls Phase 2. It could not be discharged as written.)*
...
**5. Bytes input is decoded as strict UTF-8** before clause 1; a decoding failure is `INVALID_ARGUMENT`. Decoding with replacement characters would map distinct malformed inputs onto one index value, which is a false-match primitive rather than a leniency. Pinned by the `blind-index/` `refuse` vectors since suite `0.8.0-provisional` (docs/08 §4.4; G26). A text-typed API (Python `str`, JavaScript `string`) reaches clause 1 directly.
...
| Asynchronous companions | Optional, per binding (spec §11.1, G9). Where a binding ships one it is named in that binding's own convention — the TypeScript core spells them `blindIndexAsync` and `unindexableMarkerAsync` — and these are **not** portable names: no cross-implementation test can observe a companion's shape, because only an L4 adapter written against one core in one language ever calls it. The obligations that *are* portable are §11.1's: byte-identical output, the same §9 error for the same condition, and a synchronous form that is not a blocking wait on the companion |
