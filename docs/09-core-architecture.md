...
*The pin's currency is also G16* (part C): §7.1 says the identifier changes when the pin moves but not what moves it, and before format freeze the pin can be redefined in place rather than minting `nfc-casefold-v2`, so the window is Gate 0b, not first production data.
