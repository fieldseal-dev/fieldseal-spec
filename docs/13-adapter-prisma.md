...
- **Logical type (`as:`).** The Prisma column type is the **storage** type — `Bytes` because it holds an envelope — so it cannot also declare what the value *is*. Django never needed this: `Encrypted(models.EmailField())` composes an inner field carrying the logical type, and Prisma has no equivalent. Each encrypted column therefore declares one of spec §3.6's eight logical types — `as: "string" | "bytes" | "int" | "decimal" | "float" | "boolean" | "date" | "datetime"` — defaulting to `string`, and the codec renders it exactly as §3.6 pins. `decimal` and `date` were added by G25 (#123). Before them a decimal could only be declared `float`, where it lost precision, and a date could only be declared `datetime`, where Django could not read it back. JavaScript has neither type, so both use a stated convention. A `decimal` is written as a string or a `Prisma.Decimal` and read back as its canonical string; a `number` is refused because it is a binary64 already. A `date` is a `Date` at exactly UTC midnight, and any other instant is refused rather than truncated. A `datetime` whose stored microseconds a `Date` cannot hold is refused on read rather than truncated. The `codec/` vectors (`docs/08` §4.8) pin all of it, and `tests/codec-vectors.test.ts` runs them.
...
| **Cross-language**: a row written here, decrypted by another implementation | ✅ `tests/cross/produce.ts` → `cross-prisma.json`, consumed by the Python and TypeScript cores in the N×N job. Covers the six `as:` types the fixture declares (not yet `decimal` or `date`, which the `codec/` vectors cover instead), both storage forms, the tenant-bound context and the empty string |
...
- **Cross-language sharing test — built, 2026-08-31.** `tests/cross/produce.ts` writes rows through the real extension, reads the raw columns back, and emits `fieldseal-vectors/cross/v1`; the N×N CI job has both cores decrypt it. **The index half followed on 2026-08-31** (`cross/v2`): six cases whose *index sibling column* is read raw and re-derived by a core in another language, because a mismatched index is a silent lookup miss and the envelope half cannot see it. Two of them only an adapter can produce — an index whose source column is base64 text while the sibling stays raw bytes (§7.11), and the §7.2 reserved marker the extension stored for a refused value with no caller asking for it. One thing the plan did not anticipate: Prisma is the adapter where the **codec** decision is largest, because its schema type is the *storage* type and the logical type is therefore an `as:` declaration that exists nowhere else. The producer exercises all six `as:` renderings and both storage forms rather than text plus an integer, and the test asserts each expected plaintext instead of round-tripping it — a consumer that decoded an integer differently would decrypt successfully and read the wrong value.
...
**The message's character and position come from the core, not from its error text** (G22, [#88](https://github.com/fieldseal-dev/fieldseal-spec/issues/88), closed 2026-08-31). The extension throws rather than renders, but the thrown `FieldsealUnindexable` carries what `docs/12` §10.2 needs, and until G22 closed it carried only half: the offset was recovered by a regex over the core's message that matched only the `identity`/bytes path, so on `nfc-casefold-v1` — the normalizer every indexed column here declares — `detail.offset` was always `null` and the rendered message named the character without saying where it was. Measured before the fix rather than reasoned about: `{"codePoint":"U+0378","offset":null}`. The offset is counted in **code points**, so an astral character ahead of the fault counts as one position, not two.

The character is found by offering each code point to the column's **own** declared normalizer until one is refused — the same shape as Django's `locate_unindexable`, and for the same reason. A review round caught the first attempt using the newly exported `firstUnassigned` directly: that accessor answers `nfc-casefold-v1`'s refusal rule, while `identity` refuses only unpaired surrogates and indexes unassigned code points perfectly well. On an `identity` column it therefore named a character the column handles fine, at the wrong position, offering a remedy that did not apply. The exported accessor's purpose is a caller that holds the text *before* a write and wants to know whether an `nfc-casefold-v1` column will take it; it is not a substitute for asking the column's own normalizer. A lone surrogate also gets its own wording: no Unicode version will ever assign one, so "a character we have not added support for yet" would be false.
