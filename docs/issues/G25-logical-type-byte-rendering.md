...
**Status:** ✅ **CLOSED 2026-09-18** — tracker [#123](https://github.com/fieldseal-dev/fieldseal-spec/issues/123). Resolved to direction 1, a normative vocabulary and rendering table: new spec §3.6 pins eight logical types (`string`, `bytes`, `int`, `decimal`, `float`, `boolean`, `date`, `datetime`), each with one canonical rendering. Readers are as strict as writers, and a platform that cannot hold a value exactly refuses it rather than rounding or truncating. `decimal` is canonical by value, so `1.50` and `1.5` are one plaintext and one blind-index value. `float` uses ECMAScript's `Number::toString`, with `-0` kept. `datetime` is UTC with six fractional digits. Types outside the vocabulary are refused at declaration. §10.2 names the codec as a site, and §12 adds the `codec/` family: 124 vectors under the new `MANIFEST.adapter_files`, which binds adapters rather than cores (suite `0.7.0-provisional`). Both adapters changed. Django maps inner fields to types, refuses the rest and renders canonically; Prisma gains `as: "decimal"` and `as: "date"`. Nothing had been released, so no deployment needs a backfill. Two findings came out of the fix and are in §3.6's justification: the Django codec split equal decimals into different blind-index values *within one adapter* (a silent lookup miss), and a binary64 rendered differently in CPython and ECMAScript. The demo keeps its three-type model as scope, not as a workaround (`docs/20` §7).
