...
- At least one vector at a **raised** cost — `t = 4` at the same memory is sufficient and cheap — so that "deployments may raise them" is tested rather than only stated. Every other vector uses the minima, which is exactly the configuration under which a core that hard-codes them still passes; the raised vector is the only one that can tell the two apart across implementations. Added [#62](https://github.com/fieldseal-dev/fieldseal-spec/issues/62), which fixed the Python core's inability to express a raised cost; the two cores now take it per column and each tests its own plumbing, but nothing yet asserts that they agree on the *value* at a raised cost, and only a vector can. Sequenced here, when the family was held out of both harnesses. The family was then pinned on 2026-08-31 ahead of this gap closing (a project decision, `docs/07` §7), and the raised-cost vectors shipped in the #108 review round — `raised-cost-t4-b15` at `t = 4`, plus `unindexable-marker-t4-b15` for the reserved marker — so **this obligation is met**; the others in this list stand.
