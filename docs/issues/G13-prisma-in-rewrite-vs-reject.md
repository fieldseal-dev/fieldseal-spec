...
**Status:** RESOLVED in spec 2026-08-09, adopted as proposed with one addition — docs/02 §10.2's Prisma bullet is restructured into three parts: the shared failure mode, the scoped `in:` MUST (reject *unless* rewritten to the declared blind index as §7.10 membership with §7.5 re-verification; reject whenever the rewrite cannot be guaranteed), and the unconditional `contains:`/`startsWith:` rejections. The bullet states the principle the MUST encodes — *never silently mis-serve the query*, not *never serve it correctly* — so the next adapter author can tell which side of the line a new shape falls on. The addition closes a gap the scoping opens: because §7.9 permits prefix indexes, "scope the MUST to rewriting adapters" invites someone to rewrite `startsWith:` onto one, so §10.2 now says explicitly that a prefix index is queried through its own declared predicate and that silently rewriting `startsWith:` is forbidden — the rewrite is sound only when the operand length equals the declared prefix length and is silently wrong otherwise. Marker sweep: docs/13 §2 (where-tree), §4 (throw-list preamble and the `in:` note), §6 (coverage matrix — the documented deviation is removed, not reworded). Close tracker issue [#13](https://github.com/fieldseal-dev/fieldseal-spec/issues/13) when this lands.
