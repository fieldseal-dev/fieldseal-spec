...
- **It is the project's stated reflex.** "Adapters throw rather than degrade" (`CLAUDE.md`; spec §10.2) is about not mis-serving a request silently. A `rotate` that encrypts is a correct envelope produced by the wrong operation for a reason the caller did not intend, which is the degrade, not the throw.
