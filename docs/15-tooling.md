...
| **Safe writes** | Batches go through the **encrypting path of the host language's data layer or the core directly** — never `UPDATE … SET col_ct = col` SQL. Verified-good paths per `docs/04` §11: Django `bulk_update`; SQLAlchemy `session.execute(update(User), [dicts])`. The Prisma path — per-row/`updateMany` through the extension — was *proposed* here and is now **confirmed 2026-08-27** (`docs/13` §6): `create` and `updateMany` issued on the extended client both traverse `$allOperations` in full. The obligation that came with the confirmation is that the backfill must hold the **extended** client; the base client bypasses the pipeline silently. Each batch wrapped in a short transaction; long transactions are the classic backfill outage cause |
...
- Adapters: each documents its backfill write path in its coverage matrix (`docs/12` §7 for Django and `docs/13` §6 for Prisma, both verified — the Prisma one on 2026-08-27, which retired the [VERIFY] this line used to point at).
