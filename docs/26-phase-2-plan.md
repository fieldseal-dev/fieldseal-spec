...
| [`docs/27-core-java.md`](27-core-java.md) | Java core binding (WS-I) | Written 2026-09-22 from the reviewed JVM design (§1 item 3) |
...
2. **The `docs/14` §4 amendment for unrepresentable operands (G26, tracker [#176](https://github.com/fieldseal-dev/fieldseal-spec/issues/176), filed 2026-09-22).** `docs/14` §4 has a runtime that cannot run an `out_of_band` entry record `not-run`, and a `not-run` entry blocks the level claim on the same terms as a failure. That is right for an operand that is merely too large to allocate. It is wrong for one the language cannot represent, which `docs/14` §4 already names as a category — "excluded by *representability*, which is a property of the requirement itself and will not go away" — without giving it a passing status. Read literally, the Java and .NET cores could never claim L0 (their byte arrays have `int` lengths, so a 2³¹-byte operand does not exist), and neither could Go (its string literals cannot hold the lone-surrogate operand). The need was recorded in the closure comment on #167 (2026-09-20). The direction was revised the same day after review ([comment](https://github.com/fieldseal-dev/fieldseal-spec/issues/176#issuecomment-5784498270)):
   - out-of-band status is `pass | fail | not-run`; the TypeScript harness's `not-verified` is a bug and is removed;
   - every entry carries a `basis` (`direct`, `seam`, `representability`), printed with the level claim;
   - the length bound may be proven through a **seam**, by any core that prefers it: an internal operand with a 64-bit length that every public entry point routes through, named as architecture in `docs/09` §4, with synthetic operands, zero key-provider calls, zero content reads and a bite check. `#decrypt` is defined by implied plaintext length ≥ 2³¹, not by an overhead constant;
   - the lone-surrogate entry splits. Its bytes layer becomes `blind-index/` vectors (`ED A0 80`, `ED B0 80`, `INVALID_ARGUMENT`), which settles G15's unshipped obligation and bumps the suite to `0.8.0-provisional`. Its text layer stays an out-of-band entry, recorded with `basis: representability` in Go and Rust. **Decided 2026-09-22** ([comment](https://github.com/fieldseal-dev/fieldseal-spec/issues/176#issuecomment-5784583596)): that record is `pass` with `basis: representability`, available only once the clause-5 vectors are pinned, with a `method` that states the premise and cites the vector ids, and with distinguishability asserted by the harness on the two byte inputs rather than argued.

   **Closed 2026-09-22** ([#179](https://github.com/fieldseal-dev/fieldseal-spec/pull/179), [#180](https://github.com/fieldseal-dev/fieldseal-spec/pull/180); suite `0.8.0-provisional`). It had to close before any Phase 2 core emits a report. Without it, `docs/25` §10 item 3 applies as written — a core that passes with `not-run` entries does not count toward PRD metric M1 — and the Phase 2 exit criterion is unreachable for three of the five languages (§6).
3. **The Java binding doc** ([`docs/27-core-java.md`](27-core-java.md), **written 2026-09-22**), written from the reviewed JVM design, with the JDK floor decided (§5 item 1). The design becomes the binding doc; it does not also land as a separate document. On the way in it loses what was true only when it was drafted on 2026-09-19: the premise that the `docs/09` §4 buffer-maxima flag waits on this core (#167 removed it, and #168 turned it into a per-binding obligation), every "M5 is not closed" sentence, and every reference to a file outside the repository. Its §0 review record stays, because the corrections it lists are the evidence the design was checked.
4. **The conformance tooling grows from two cores to N** (WS-L). `tools/conformance/compare_result_ids.py` reads exactly two reports today (`docs/14` §2), and the `cross-consume` job's consumer matrix is `[python, typescript]`. Both are generalised, with their tests, before a third core arrives, so that the third core's arrival is a one-line matrix change and not a redesign under pressure. **Done 2026-09-22** ([#182](https://github.com/fieldseal-dev/fieldseal-spec/pull/182)). It is not one line: a core still brings its own jobs and steps, because producing and consuming are per-language code. What it no longer needs is any change to the comparison or to the existing consumers, and the list of what it does need is written above `cross-produce` in `conformance.yml`.
...
- **Java and .NET:** the `docs/09` §4 length-bound seam (G26) is part of the core, not an option. `byte[]` lengths are `int` on both platforms, so the seam is their only route to a `pass` on `spec/3.5/length-bound` and `#decrypt`, and the `docs/09` obligation is unexercised until they exist.
