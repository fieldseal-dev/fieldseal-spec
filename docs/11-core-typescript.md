...
| Argon2id **synchronous** raw output | **Open decision — the hardest dependency choice in either Phase 1 core**, and now partly constrained by whether Argon2's secret key `K` must be supplied (see [`G02`](../docs/issues/G02-argon2id-parameters.md) portability finding, 2026-08-22). Candidates: `sodium-native` (`crypto_pwhash` is sync, raw output, explicit salt — but **exposes no secret parameter at all**, so it is disqualified outright if G2 keeps `K`) · `@node-rs/argon2` (**[VERIFY]** sync raw-output API; **[VERIFY]** `secret` support) · npm `argon2` / node-argon2 (**does** expose `secret`, documented as the "pepper"; async-only API **[VERIFY]** — unusable for the sync §11.1 `blind_index` if so) | Abstract behind an internal `Argon2Backend` interface so the choice is swappable without API change. Note the asymmetry this creates: the TS ecosystem can supply `K` and the Python one cannot, which is the wrong way round for a portability claim |
