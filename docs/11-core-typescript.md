...
- **What crosses the `await` in a companion.** `idfAsync` derives the 16-byte Argon2id salt *before* its first `await`, so both the tenant index key's copy and the per-column index key are erased before the threadpool derivation is queued; what remains in flight is that salt and a private copy of the normalized value. The copy is not redundant and must not be refactored away: under `identity` the normalized value **is the caller's own array**, and on the marker path it is the process-wide `UNINDEXABLE_PREIMAGE` singleton — zeroing either would destroy a caller's buffer or every subsequent marker in the process. `tests/async-companions.test.ts` holds both cases, and node's own internal copies remain outside this core's reach exactly as the zeroization-honesty bullet above says.
