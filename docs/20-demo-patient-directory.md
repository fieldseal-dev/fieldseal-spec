...
**Three logical types, not eight.** The shared model uses `string`, `int` and `bytes` only. When the demo was built, the other types did not survive a trip between these two adapters — [G25](issues/G25-logical-type-byte-rendering.md), measured in three different failure classes. A `date` Django wrote was read by Prisma as an instant, and one ordinary rewrite through Prisma stored a form Django could no longer read, with nothing raised when the damage was done. A `Decimal` had no Prisma declaration, so it went through a binary64: `b"12345678901234567.89"` came back as `12345678901234568`. A `boolean` was `b"True"` on one side and `b"true"` on the other, and each refused the other.

**That gap is closed.** Spec §3.6 (2026-09-18) pins one rendering for each of eight logical types, both adapters implement it, and the `codec/` vector family checks every rule in both directions. The restriction stays as the demo's *scope*: widening the shared model changes the checked-in transcript, so it is a change of its own. `check_declarations.py` still refuses any other inner type until that change is made, so a column cannot be added outside it by accident.
