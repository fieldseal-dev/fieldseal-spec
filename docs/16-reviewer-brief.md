...
| 2026-08-31 | Prof. Kenneth G. Paterson (ETH Zurich, Applied Cryptography Group), senior author of the USENIX Security '23 MongoDB QE analysis | [Q5](#q5), with [Q1](#q1)/[Q2](#q2) named as the fuller ask | **Sent.** Awaiting response. Chosen because the group analysed leakage from a shipping encrypted database and Q5 is a leakage/oracle question about a decrypt path that deliberately refuses to collapse its errors; spec §2.3 leans on their `queryLog`/`opLog` recovery result. Addressed to the senior author — the paper's title page marks no corresponding author |
