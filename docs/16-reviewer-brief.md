...
- **Gate 0b — freeze.** This is still you, and it is unchanged. Until two credentialed reviewers have read the spec and their objections are addressed or documented, the project does not assign a real suite identifier, does not publish a stable vector suite, does not claim conformance to a frozen format, does not publish anything at 1.0, does not invite production adoption, and does not run its dissemination track. Pre-1.0 experimental releases for evaluation are permitted under PRD §8's conditions (2026-09-18). They make the code you would be reviewing installable, and they change nothing you would be asked to decide.
...
**What closes it:** confirmation that the chosen encoding is injective over the current field set *and* over plausible future extensions — or a demonstrated aliasing. Still the cheapest question here, and the one where a counterexample is most likely.

The first half now has one round of external confirmation and no counterexample (below), so **the live half is extension**, and it has a concrete proposal to rule on rather than an open-ended ask — [`issues/G04`](issues/G04-tenant-id-null-encoding.md) option 5: with no decoder anywhere in the design, is *"a presence bit's meaning is immutable once assigned, an absent optional field always means not-bound, and bit 7 of the bitmap is a continuation flag"* sufficient for cross-version injectivity, or does the encoding need a version field of its own despite the `fmt_ver` and `suite_id` it already carries? A reviewer who wants the byte should say which variant they mean — G04's 4a (encoder-pinned; separates versions, and thereby forbids reading older data in place) and 4b (dispatched off the envelope's `fmt_ver`; preserves mixed-version reads and separates nothing) are different answers, and the distinction is where this question actually turns.

**Input received, 2026-08-26 (one Crypto Stack Exchange answer to the 2026-08-25 post; not a review, recorded as what it is).** Maarten Bodewes, site moderator:

- *On the core argument:* "Looks correct to me" — length/value with a fixed-width length is fine, and a presence bit per optional field is enough to separate absent from present-and-empty; he prefers this notation over magic values. The "no parser exists" subtlety was not engaged as a distinct point; the argument was accepted as an encoder property without qualification.
- *On extension:* answered for the with-decoder case only — a version prefix is fine "as long as the decoder can handle both versions." This encoding has no decoder (§6.2: produced and recomputed, never parsed), so the sub-question actually asked — whether "new optional fields consume new presence bits" suffices across versions, or whether the 8-bit bitmap needs its own version/length prefix before bit 7 is consumed — was not ruled on and remains open.
- *Offered unprompted:* TLV alternatives — ASN.1 DER with `IMPLICIT`-tagged optional fields (sketched against this exact field set) or CBOR — and the observation that fixed `u64be` length prefixes are wasteful.

**Follow-up, 2026-08-28.** A comment restated the encode-only case explicitly — there is no decoder, ever; the bytes only feed a KDF and an AEAD's AAD and are recomputed on both ends, so "as long as the decoder can handle both versions" has no referent here — and asked whether "a new optional field takes the next free presence bit" alone suffices for cross-version injectivity, or whether the bitmap needs a version/length prefix now, before bit 7 is consumed and a second presence byte would shift every later field. Answered the same day:

> "I would strongly advice a single byte version in there, so that this kind of thing doesn't even come up. However, you could argue that the absence of a field is enough for you. But this is kind of the same thing as assigning meaning to a `null` value, if you know what I mean. Cryptographically it can be secure if semantically it means the same thing, basically. Having a static versioning byte lets you skip that discussion."

(The answer was edited a minute later, adding that DER is itself a canonical encoding, used by X.509v3 — a point about the TLV alternative, not about extension.)

What that gives, stated as what it is:

- **Still no counterexample.** Across the answer and the follow-up, no aliasing was produced — not against the current field set, not against the extension rule.
- **The extension half is answered conditionally, not cleanly.** Bit-level injectivity across versions is not disputed: an old encoder cannot set a bit it does not know. But the guarantee is made to rest on the semantics of absence holding still — "cryptographically it can be secure if semantically it means the same thing." The residual risk is therefore not a byte collision within one version; it is a later version giving an unset bit a *different* meaning (a default other than "not bound"), at which point one byte string binds two distinct contexts.
- **The version byte is offered as hygiene, not as a repair.** "Lets you skip that discussion" is the whole of its stated justification. Adopting it changes the §6.2 layout and invalidates every vector and both cores' outputs, so it is a Gate-0b decision, not something to take on engineering judgment.

One thing the exchange sharpens, recorded for whoever takes this question. Three version anchors already sit near this encoding, and none of them is the one a static version byte would add:

- **`fmt_ver` is in the AAD** (§6.2) — but on decrypt it is read *from the envelope*, so it binds the version the writer claimed, not the encoding rules the reader applied. A relocated v1 envelope carries its own `0x01` with it, so `fmt_ver` does not separate a v1 context from a v2 context that encodes to the same bytes.
- **Record-key derivation (§5.3) has no version anchor at all**: `salt = key_id ‖ msg_seed`, `info = canonical_context`. Both inputs are per-deployment or per-write; neither names a format.
- **Index-key derivation (§7.2) does have one** — the salt is the literal `"fieldseal-index-v1"` — but it versions the derivation label rather than the context encoding, and bumping it invalidates every stored blind index.

`suite_id` is also already the first field *inside* `canonical_context`, so a rule that no §6.2 encoding change may reuse a `suite_id` would separate versions without adding a field at all — but nothing says that today: design commitment 1 defines a suite as AEAD + nonce policy + KDF + index construction, and context encoding is not on that list. Neither option is decided here.

Consequence for this question: the first half of "what closes it" is confirmed by a credible practitioner and the second half is now answered conditionally, with a recommendation attached and no counterexample anywhere in the exchange. That is real input and it is logged as such — but it is one practitioner on a Q&A site, not a review, and it does not close a Gate-0b question. [Issue #4](https://github.com/fieldseal-dev/fieldseal-spec/issues/4) stays open and §6.2 keeps its `[PROVISIONAL]` marker.

What the exchange did produce is a decidable proposal where there was an open-ended question. [`issues/G04`](issues/G04-tenant-id-null-encoding.md) now carries the version byte as option 4 — split into the encoder-pinned and envelope-dispatched variants, which differ in whether they separate anything at all — against option 5, an extension rule that changes no byte and regenerates no vector. Option 5 is proposed there; neither is adopted, and nothing has been written into §6.2.
...
| 2026-08-22 | Paragon Initiative (`security@paragonie.com`), for Scott Arciszewski | [Q3](#q3), [Q6](#q6) | **Sent. No response** as of 2026-09-18, four weeks on; no follow-up was sent, including for the v0.1.2 release. Chosen because one person authored both the CipherSweet blind-index construction Q3 diverges from and the expired `draft-irtf-cfrg-xchacha` behind Q6 |
| 2026-08-25 | [Crypto Stack Exchange](https://crypto.stackexchange.com/questions/119892/is-a-presence-bitmap-plus-length-prefixed-concatenation-injective-enough-to-be-u) | [Q4](#q4) | **Posted.** Asked as a self-contained encoding question naming no project and linking no repository — §4 lists this channel for Q4 precisely because it needs no project context, and a question that reads as promotion gets closed rather than answered. The post gives the presence-bitmap encoding, the injectivity argument §6.2 states, and the two things actually asked: whether the argument survives having no decoder, and whether "new optional fields consume new presence bits" is enough to keep it injective across extensions before the bitmap runs out at bit 7. **Answered 2026-08-26** — one answer (Maarten Bodewes, site moderator), recorded under [Q4](#q4): confirms the injectivity argument as stated; the extension sub-question was answered only for the with-decoder case this design does not have. **Follow-up comment posted 2026-08-28** pressing the encode-only case, **answered the same day**: a single static version byte "strongly advised" as hygiene, with the encode-only argument conceded conditionally ("cryptographically it can be secure if semantically it means the same thing") and still no counterexample. Recorded under [Q4](#q4); the question stays open |
...
5. **Funded review — and this entry previously overstated it.** It read "OSTIF brokers audits for open-source projects at no cost to the project," which is true only in the sense that matters least here: OSTIF manages and scopes audits that *a funding body pays for*. Its own [audits](https://ostif.org/audits/) and [sponsorship](https://ostif.org/sponsorship/) pages describe corporate sponsors earmarking funds for specific projects, and the engagements it reports reaching projects through funders — Eclipse Foundation, the Sovereign Tech Fund, Alpha-Omega via its [OpenSSF membership](https://openssf.org/blog/2025/06/13/case-study-ostif-improves-security-posture-of-critical-open-source-projects-through-openssf-membership/) — rather than through a project-side request. It does not remove the budget question; it **is** the budget question, plus a broker who is good at scoping. Combined with the standing caveat that its published work is overwhelmingly shipping code rather than specifications, this is **not a route to a free cryptographic review of a pre-alpha document**, and it is not counted on. What remains under this heading is the honest floor it always had: a scoped spec review from a boutique firm, which turns Gate 0b from a recruitment problem into a budget decision.

   *Not a substitute, and named here so nobody mistakes one for the other:* the OpenSSF public tooling — [Scorecard](https://github.com/ossf/scorecard) and the [securing-software-repos working group](https://github.com/ossf/wg-securing-software-repos) — is free and worth adopting, but it measures **repository and supply-chain posture**: branch protection, pinned dependencies, signed releases, CI hygiene. It cannot read a specification, and nothing it reports moves Gate 0b, which needs credentialed humans reading §5–§7. Adopting it improves a different axis, and this brief does not claim otherwise.
