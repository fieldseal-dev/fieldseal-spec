...
**The queryset layer is not an interception point (verified; [#118](https://github.com/fieldseal-dev/fieldseal-spec/issues/118)).** Two routes reach a compiled lookup without passing a custom `QuerySet` at all. `QuerySet.complex_filter(Q)` calls `query.add_q` directly rather than `_filter_or_exclude` — only its *dict* branch routes through it — and `Model._base_manager` is a plain `django.db.models.Manager` returning a plain `QuerySet`, built that way deliberately so Django's internals are not filtered by a custom default manager. Django walks both itself: `limit_choices_to` is applied at five sites — `ForeignKey.validate` (`_base_manager.complex_filter`), `Field.get_choices` and `ForeignObjectRel.get_choices` (`_default_manager.complex_filter`), admin autocomplete, and `forms/models.py`'s `apply_limit_choices_to_to_formfield` (`_base_manager`, inside an `Exists`). So any obligation an adapter records at the queryset layer — §7.5 re-verification, or a refusal that needs queryset state — needs a backstop at the *lookup* layer, which is the only place every route passes. The lookup can tell the routes apart, because a custom queryset can mark its own `Query` and `Query.clone` copies `__dict__`, so the mark survives into subqueries and into the `Query` subclasses `chain()` swaps in. **The mark alone is not sufficient, and that is the part which is easy to miss:** it says a custom queryset owns the `Query` the lookup compiles for, not that it is the queryset answering the statement. `Exists(qs)`/`Subquery(qs)` keep `qs.query`, so a plain outer query can embed a marked inner one and be answered from it — the backstop has to read `Query.subquery` as well, or every shape it refuses is rewritable as a correlated `Exists`.
