...
The mark in the header, `static/favicon.svg`, `static/favicon.ico`,
`static/apple-touch-icon.png` and `static/brand/` are all generated by
[`tools/brand/make_logo.py`](../tools/brand/README.md) from one geometry. Unlike
`static/figures/` they are committed, because the build does not run the
generator. Regenerate rather than edit them. The header reads
`static/brand/fieldseal-mark.svg` at build time and swaps its fill for
`currentColor`, so it follows `--accent` in both themes.
