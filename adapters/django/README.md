...
| Subquery embedding (`__in=qs`, `Subquery`, `Exists`) | 🛑 refused — the outer query would receive unverified candidates. Two layers: `resolve_expression` for `__in=qs`, and `annotate()`/`alias()` for `Exists`/`Subquery`, which keep `qs.query` and never reach it | `test_a_verifying_queryset_refuses_to_become_a_subquery`, `test_a_verifying_queryset_in_an_annotation_refuses_too` |
