...
/**
 * Resolve the model a relation edge points at, refusing a miss.
 *
 * A fresh map cannot miss: the generator emits every model in the schema,
 * declared or not. So a relation naming a model the map does not carry means
 * the map is stale or was edited -- the same verdict the extension reaches for
 * an unmapped *top* model -- and walking past it would reopen that bypass one
 * hop down: a nested write through the edge stores plaintext, a read returns
 * raw envelope bytes, a filter goes unexamined, all silently.
 */
export function relationTarget(
  ...
  const target = map.byModel.get(rel.model);
  if (target === undefined) {
    throw new FieldsealConfigurationError(
      `fieldseal: ${model.model}.${rel.field} relates to "${rel.model}", which ` +
        `is not in the field map (generated by ${map.source.generator}). The ` +
        `generator emits every model in the schema, so a missing one means the ` +
        `map is stale or was edited -- re-run \`prisma generate\`. It is ` +
        `refused rather than skipped because a skipped relation is the ` +
        `unmapped-model bypass one hop down: a nested write through it would ` +
        `store plaintext with nothing raised.`,
    );
  }
  return target;
}
