# @fieldseal/prisma reference

The detail behind [`README.md`](README.md): why declarations come from a
generator, how key acquisition in the query path works, what the index can and
cannot answer in Prisma, what every query path does today and the test that
proves it, and development setup. The README is the place to start; this file
is for when a behaviour surprises you, or you are reviewing the adapter. The
design it implements is [`docs/13-adapter-prisma.md`](../../docs/13-adapter-prisma.md).

Everything here describes an experimental, unreviewed pre-1.0 release; the
README's warning applies to all of it.

## No cryptography in this package
...
## Why the index sibling is optional and never unique
...
## `Bytes` columns and the `as never` cast

An encrypted `Bytes` column makes Prisma generate `Uint8Array` for it, while the
value written is whatever `as:` declares, so the README's write examples cast.
...
block that `docs/14` §4 requires to mirror this file's coverage-matrix table.
